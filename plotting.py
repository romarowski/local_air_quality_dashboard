...
def plot_weekly_traffic_trend(df, lookback = 5):
    #df is operations grouped by week
    #lookback is how many weeks are plotted
    df = df.tail(lookback).reset_index()
    fig = px.scatter(df, x='Date', y='Operation Count') 
    fig.update_traces(mode='lines+markers')
    weekly_change = df['Operation Count'].pct_change().iloc[-1]
    #fig.update_layout(margin={"r":0,"t":30,"l":0,"b":0})
    text = traffic_text(weekly_change)
    return fig, text

def traffic_text(weekly_change):
    if weekly_change > 0:
        text = "Traffic is up " + str(round(weekly_change*100)) + "% this week"
    else:
        text = "Traffic is down " + str(round(weekly_change*100)) + "% this week"
    return text
...
def AQI_map(lat_lon_df, pollutants_df, map_type):
        ...
        poll_df = poll_df[(poll_df['Month Index']==month) & 
                  (poll_df['Year']==year)].groupby('Station').mean()
        
        coords_df['AQI'] = poll_df['AQI'].round(1)
        coords_df.fillna(0)
        ...
        fig = px.scatter_mapbox(coords_df, lat="lat", lon="lon", color='AQI',
             ...
             color_continuous_scale=[(0.00, "green") , (0.125, "green") ,       
                                     (0.125, "yellow"), (0.25, "yellow"),
                                     (0.25, "orange"), (0.375, "orange"),
                                     (0.375, "red")  , (0.50, "red")   ,
                                     (0.50, "purple"), (0.75, "purple"), 
                                     (0.75, "brown") , (1.00, "brown")],
             range_color = [0, 400],
        ...
        fig.update_layout(coloraxis_colorbar=dict(
            tickvals=[0, 25, 50, 75, 100, 125, 150, 175, 200, 250, 300,
                350, 400],
            ticktext=['0', 'Good', '50', 'Moderate', '100', 
                'Unhealthy for sensitive groups', '150', 'Unhealthy',
                '200', 'Very Unhealthy', '300', 'Hazardous', '400'],
        ))
        fig.update_layout(mapbox_style=map_type)  
        fig.update_layout(title_text='Air Quality Index for: ' +\
                obj.strftime('%B') + ' ' + str(year),
                          title_x=0.1, title_y=.97)
        fig.update_layout(margin={"r":0,"t":30,"l":0,"b":0})
        fig.update_traces(marker=dict(size=12))
...
def plot_wind_speed_direction(date_value, hour, site, df_p):
        ...
        fig.update_layout(title='Wind Speed ' + 
                str(np.round(poll_df['Wind Speed (km/h)'].to_numpy()[0],2)) +
                    'km/h towards ' + direction)
        return fig
...
def plot_pollutant_hour_date(date_value, hour, site, lat_lon_df, pollutants_df,
        pollutant, traffic_df, run_df, map_style):
   ...
   #shown in Analysis tab
    ...
    fig.update_layout(margin={"r":0,"t":30,"l":0,"b":0}) 
    fig.update_layout(title_text='There was a ' + pollutant + ' maximum at ' + site + ' on ' + date_string + ' at  ' + str(max_hour) + 'h' ,
                      title_x=0.1, title_y=.97)
    fig.update_traces(marker=dict(size=12))
...
def AQI_map_date(start_date, end_date, lat_lon_df, pollutants_df, map_style):
...
def plot_traffic_date(traffic_df, start_date, end_date):
    ...
    pctge_flights_per_terminal = df_month['Air Terminal'].\
            value_counts(normalize=True) * 100
    df_plot = pctge_flights_per_terminal.to_frame('perc').reset_index()
    df_plot['Count'] = df_month['Air Terminal'].value_counts().to_numpy()
    fig = px.bar(df_plot, x='index', y='perc',
             labels={'index' : 'Terminal', 'perc': 'Number of Flights [%]'},
             custom_data = ['Count'],
             title='Percentage of Flights per Terminal from ' +\
             start_date_string + ' to ' + end_date_string)
    fig.update_traces(
    ...
    #pdb.set_trace()
    pctge_flights_per_runway = df_month['Runway'].\
            value_counts(normalize=True) * 100
    df_plot = pctge_flights_per_runway.to_frame('perc').reset_index()
    df_plot['Count'] = df_month['Runway'].value_counts().to_numpy()
    fig2 = px.bar(df_plot, x='index', y='perc',
             labels={'index' : 'Runway', 'perc': 'Number of Flights [%]'},
             custom_data = ["Count"],
...
def plot_traffic_bars(start_date, end_date, traff_df):
    ...
    #Prepare datasets
    df = traff_df.sort_values(by='To Date').reset_index(drop=True)
    df = df[(df['To Date']>=start_date) & (df['To Date']<=end_date)]
    df_s = traff_df
...
def plot_pollutant_multi_stations_date(start_date, end_date, pollutant, poll_df, stations, 
    ...
    lim = limits_df[limits_df['Pollutant'] == pollutant]['Limit SA']
